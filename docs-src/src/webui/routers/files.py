...
def _entry_info_from_scandir(entry: os.DirEntry) -> Dict[str, Any]:
...
# ---------------------------------------------------------------------------
# GET /v1/webui/files/list?path=/&offset=0&limit=200
# ---------------------------------------------------------------------------

async def files_list(request: aiohttp.web.Request) -> aiohttp.web.Response:
    """List directory contents at the given path with pagination.
    ...
    The full directory listing is sorted cheaply using :func:`os.scandir`
    cached attributes (directories first, then alphabetical by name).
    Only the requested slice is stat'd for detailed size/mtime information.

    Response includes ``total`` (total entry count in the directory),
    ``offset``, and ``limit`` so the client can implement lazy loading.
    """
    ...
    # Use os.scandir for performance (caches stat results on DirEntry).
    try:
