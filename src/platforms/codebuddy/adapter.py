...
# 能力字典——从 test.py 的 SUPPORTED_CAPABILITIES 映射
CAPS: Dict[str, bool] = {
    "chat": True,
}
...
class CodebuddyAdapter(PlatformAdapter):
    ...
    通过 CodeBuddy API 访问大模型。
    每个 token/user_id 对作为一个独立候选项，由 Provider-V2 核心 TAS 算法调度。
    """
    ...
    async def init(self, session: aiohttp.ClientSession) -> None:
        ...
        Returns:
            无返回值。
        """
        from src.platforms.codebuddy.client import CodebuddyClient

        self._client = CodebuddyClient()
    ...
    async def _background_init(self) -> None:
        ...
        try:
            ...
            logger.warning("codebuddy 后台初始化失败: %s", exc)
    ...
    async def _on_models_updated(self, models: List[str]) -> None:
        ...
        logger.info("codebuddy 模型列表已更新: %d 个", len(models))

    async def fetch_remote_models(self) -> List[str]:
        """拉取远程模型列表。

        CodeBuddy 平台模型列表固定，直接返回硬编码列表。
    ...
    async def close(self) -> None:
        ...
        if self._refresh_task is not None and not self._refresh_task.done():
            ...
            try:
                ...
                logger.debug("codebuddy refresh task cancelled")
        if self._client is not None:
