...
from src.platforms.chutes.accounts import (
    API_KEYS,
    AVAILABLE_MODELS,
    CHAT_URL,
    MAX_TOKENS_NON_STREAMING,
...
class _KeyState:
    ...
    def mark_failure(self, status: int = 0) -> None:
        ...
        if status in (401, 402, 403):
            self.valid = False
            logger.warning("Chutes Key 无效 (HTTP %d): %s...", status, self.key[:16])
        else:
...
class ChutesClient:
    ...
    async def complete(
        ...
        **kw: Any,
        ...
        """调用 Chutes chat completions 端点"""
        ks = self._find_key(candidate)
        if not ks:
            raise PlatformError("Chutes: 未找到对应 API Key")
        ...
        max_tok = MAX_TOKENS_STREAMING if stream else MAX_TOKENS_NON_STREAMING
        if kw.get("max_tokens") is not None:
            max_tok = kw["max_tokens"]
        ...
        try:
            async with self._session.post(
                ...
                if resp.status != 200:
                    ...
                    raise PlatformError(
                        f"Chutes HTTP {resp.status}: {body[:300]}"
                    )
            ...
            async with self._lock:
                ks.mark_failure(0)
            raise PlatformError(f"Chutes 请求失败: {e}") from e
