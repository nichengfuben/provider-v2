...
class ModelsCache:
    ...
    def __init__(
        ...
        Args:
            platform: 平台标识名。
            fallback_models: 兜底模型列表。
            fetch_enabled: True=覆盖策略，False=只增不减。
        """
    ...
    async def load(self) -> List[str]:
        ...
        try:
            ...
            logger.warning("[%s] 模型缓存加载失败: %s", self._platform, exc)
    ...
    async def save(self, models: List[str]) -> None:
        ...
        try:
            ...
            logger.warning("[%s] 模型缓存保存失败: %s", self._platform, exc)
    ...
    async def _do_refresh(
        ...
        try:
            ...
            logger.warning("[%s] 模型列表刷新失败: %s", self._platform, exc)
        finally:
            self._refreshing = False
