"""Yandex Translate HTTP 客户端。

负责 API Key 管理、候选项构造与翻译请求。
使用 Yandex Cloud Translate API (v2)。
"""
...
class _KeyState:
    ...
    def mark_failure(self, status: int = 0, message: str = "") -> None:
        ...
        if status in (401, 403) or "unauthenticat" in message.lower():
            self.valid = False
            logger.warning(
                "yandextranslate Key 无效 (HTTP%d): %s... | %s",
                status,
                self.key[:12],
                message[:100],
            )
        elif status == 429 or "resource_exhausted" in message.lower():
            self.rate_limit_until = time.time() + RATE_LIMIT_COOLDOWN
            logger.warning("yandextranslate Key 限速: %s...", self.key[:12])
...
class YandexTranslateClient:
    ...
    async def candidates(self) -> List[Candidate]:
        ...
        return [
            Candidate(
                id=make_id("yandextranslate", ks.key[:12]),
                platform="yandextranslate",
